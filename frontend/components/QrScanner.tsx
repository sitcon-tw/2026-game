...
export default function QrScanner({ onScan, scanStatus, alternateContent, showAlternate = false }: QrScannerProps) {
		...
		<div className="relative h-[300px] w-[300px] rounded-lg overflow-hidden bg-[#6b6b6b]">
			...
			{showAlternate && alternateContent ? (
				...
				<Scanner
					...
					constraints={selectedDeviceId ? { deviceId: { ideal: selectedDeviceId } } : { facingMode: { ideal: "environment" } }}
					styles={{
						container: {
							width: "300px",
							height: "300px"
			...
			{showScanner && scanStatus.type === "scanning" && (
				<div className="absolute inset-0 z-20 flex items-center justify-center bg-black/40">
					<div className="rounded-lg bg-white px-6 py-3 text-lg font-bold text-[var(--text-primary)]">處理中…</div>
				</div>
			)}
			{showScanner && scanStatus.type === "success" && (
				<div className="absolute inset-0 z-20 flex items-center justify-center bg-black/40">
					<div className="rounded-lg bg-green-500 px-6 py-3 text-lg font-bold text-white">{scanStatus.message}</div>
				</div>
			)}
			{showScanner && scanStatus.type === "error" && (
				<div className="absolute inset-0 z-20 flex items-center justify-center bg-black/40">
					<div className="rounded-lg bg-red-500 px-6 py-3 text-lg font-bold text-white text-center">{scanStatus.message}</div>
				</div>
			)}
		</div>
	);
