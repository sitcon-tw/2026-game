...
export default function Header() {
  ...
  const progressPercent =
    unlockLevel > 0 ? (currentLevel / unlockLevel) * 100 : 0;
      ...
      <div className="relative flex h-full flex-col justify-center px-6">
        <div className="flex items-center justify-between gap-4">
          <div className="flex flex-[2] flex-col gap-1">
            <div className="text-[var(--text-gold)] font-serif text-xl font-semibold tracking-wide">
              SITCON 大地遊戲
            </div>
            <div className="font-serif text-lg">
              {rank != null ? (
                <span className="text-[var(--text-light)]/90">
                  第 {rank} 名
                </span>
              ) : (
