...
export default function UnlockMethodCard({ title, current, total, loaded = false, onClick }: UnlockMethodCardProps) {
	...
	return (
		...
		>
			...
			<div className="flex flex-col items-center gap-3">
				{/* Title */}
				<div className="text-[var(--text-light)] text-2xl font-serif font-bold text-center">{title}</div>

				{/* Progress Bar */}
				<ProgressBar percent={progress} variant="bronze" height="h-3" loading={isLoading} className="w-40" />
			...
			<div className="w-14 h-14 flex items-center justify-center">
				<Image src="/assets/challenge/arrow-right.svg" alt="arrow right" width={38} height={38} />
			</div>
		</button>
	);
}
