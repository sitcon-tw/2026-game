	...
	return (
		...
		>
			...
			<div className="grid grid-cols-[1fr_auto] items-center gap-4">
				<motion.button type="button" onClick={onEdit} className="flex cursor-pointer items-center gap-3 min-w-0 text-left" whileTap={{ scale: 0.95 }}>
					{avatar ? (
						<img src={avatar} alt={nickname ?? "我"} className="h-14 w-14 rounded-full object-cover flex-shrink-0" />
					) : (
						<div className="flex h-14 w-14 items-center justify-center rounded-full bg-[var(--accent-bronze)] text-xl font-bold text-white flex-shrink-0">{(nickname ?? "我").charAt(0)}</div>
					)}
					<div className="min-w-0">
						...
						<p className="text-sm text-[var(--text-secondary)]">第 {currentLevel ?? "?"} 關</p>
					</div>
				</motion.button>
			...
			>
				...
				<p className="mt-1 whitespace-pre-wrap text-sm text-[var(--text-primary)]">{bio?.trim() || "尚未填寫"}</p>
			</motion.button>
			...
			>
				...
				<p className="mt-1 break-all text-sm text-[var(--text-primary)]">{email || "尚未公開"}</p>
			</motion.button>
			...
			>
				...
				{links.length > 0 ? (
					<div className="mt-1 space-y-1">
						{links.map(link => (
							<p key={link} className="break-all text-sm text-[var(--bg-header)] underline">
								{link}
							</p>
						))}
					</div>
