...
import type { AdminLoginResponse, DiscountCoupon, DiscountCouponGift, User } from "@/types/api";
...
export function useAdminCreateGiftCoupon() {
	...
	return useMutation({
		mutationFn: (payload: { discount_id: string; price: number }) => api.post<DiscountCouponGift>("/admin/gift-coupons", payload),
		onSuccess: () => {
...
export function useAdminAssignCoupon() {
	return useMutation({
		mutationFn: (payload: { user_id: string; discount_id: string; price: number }) => api.post<DiscountCoupon>("/admin/discount-coupons/assignments", payload)
	});
}
