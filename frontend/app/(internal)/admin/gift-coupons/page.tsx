...
import { useAdminCreateGiftCoupon, useAdminDeleteGiftCoupon, useAdminGiftCoupons } from "@/hooks/api";
...
function CreateGiftCouponForm() {
	...
	return (
		<form onSubmit={handleSubmit} className="flex flex-col gap-3">
			<h2 className="font-serif text-lg font-bold text-[var(--text-primary)]">新增 Gift Coupon</h2>
	...
	return (
		<div className="flex flex-col gap-2 rounded-xl bg-[var(--bg-secondary)] p-4">
			<div className="flex items-center justify-between">
				<span className="font-serif text-lg font-bold text-[var(--text-primary)]">${gift.price}</span>
				<button
					onClick={() =>
						deleteGift.mutate(gift.id, {
							...
							onError: error => {
								...
								showPopup({
									title: "刪除失敗",
									description: notFound ? "此票券已被兌換，無法刪除。請重新整理頁面" : `${error?.message ?? "未知錯誤"}，請重新整理頁面後再試`
								});
							}
						})
					}
			...
			<p className="text-xs text-[var(--text-secondary)]">Discount ID: {gift.discount_id}</p>

			<div className="flex items-center gap-2">
				<code className="flex-1 overflow-hidden text-ellipsis rounded-lg bg-[var(--bg-primary)] px-3 py-2 text-xs text-[var(--text-primary)]">{gift.token}</code>
				<button onClick={handleCopy} className="shrink-0 rounded-lg bg-[var(--bg-primary)] px-3 py-2 text-xs font-medium text-[var(--text-secondary)] transition-transform active:scale-95">
					{copied ? "已複製" : "複製"}
				</button>
			</div>
		</div>
	...
	return (
		<div className="flex flex-col gap-6">
			...
			<div className="flex flex-col gap-3">
				<h2 className="font-serif text-lg font-bold text-[var(--text-primary)]">Gift Coupons ({gifts?.length ?? 0})</h2>
				...
				{!isLoading && gifts?.length === 0 && <p className="text-sm text-[var(--text-secondary)]">尚無 gift coupons</p>}
