...
import {
  ...
  Staff,
  StaffScanAssignmentItem,
} from "@/types/api";

type StaffTab = "redeem" | "assign";

function ScanAssignmentHistoryList({
  history,
  isLoading,
}: {
  history: StaffScanAssignmentItem[];
  isLoading: boolean;
}) {
  if (isLoading) {
    return (
          ...
          {history.map((item, idx) => (
            ...
            >
              <div className="flex items-center justify-between gap-3">
                <span className="truncate font-serif font-bold text-[var(--text-primary)]">
                  {item.nickname || "未知玩家"}
                </span>
                <span className="shrink-0 rounded-full bg-[#59360B]/10 px-2 py-1 text-xs font-medium text-[#59360B]">
                  {item.discount_id}
                </span>
              </div>
...
function StaffScanContent() {
  ...
  // Staff store
  const { staffToken, setStaffToken, setStaffName, clearStaff } =
    useStaffStore();
  ...
  const { data: redemptionHistory, isLoading: historyLoading } =
    useStaffRedemptionHistory();
  const { data: scanAssignmentHistory, isLoading: scanHistoryLoading } =
    useStaffScanAssignmentHistory();
  ...
  useEffect(() => {
    ...
    if (tokenFromUrl) {
      ...
      staffLogin.mutate(tokenFromUrl, {
        ...
        onError: (error) => {
          const message = translateWithContext(
            "staff-login",
  ...
  const handleRedeemScan = useCallback(
    (result: { rawValue: string }[]) => {
      ...
      if (
        redeemScanStatus.type === "scanning" ||
      ...
      staffLookupCoupons.mutate(userCouponToken, {
        ...
        onError: (error) => {
          ...
          showPopup({
            ...
            description:
              apiMessage ||
              "查詢折扣券失敗。可能的原因：玩家持個人 QR Code，請確認玩家切換至折價券專用兌換 QR Code。",
          });
          setRedeemScanStatus({ type: "idle" });
        },
      });
  ...
  const handleAssignScan = useCallback(
    (result: { rawValue: string }[]) => {
      if (!result.length) return;
      if (
        assignScanStatus.type === "scanning" ||
        staffAssignCouponByQRCode.isPending
      )
        return;
  ...
  const currentScanStatus =
    activeTab === "redeem" ? redeemScanStatus : assignScanStatus;
  ...
  return (
    <div className="flex flex-1 flex-col items-center px-6 py-8">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-red-600">
                {loginErrorMessage || "工作人員登入失敗，請確認連結是否正確"}
              </p>
            </div>
          </div>
        </div>
      ...
      {activeTab === "redeem" ? (
        <p className="mt-3 max-w-[340px] text-center text-sm text-[var(--text-secondary)]">
          掃描玩家的折價券 QR Code，先查詢可用折價券後再進行核銷
        </p>
      ) : (
        <p className="mt-3 max-w-[340px] text-center text-sm text-[var(--text-secondary)]">
          掃描玩家的一次性 QR Code，發放固定折價券（每位玩家僅能領取一次）
        </p>
      )}
      ...
      {activeTab === "redeem" ? (
        ...
        <ScanAssignmentHistoryList
          history={scanAssignmentHistory ?? []}
