...
function BoothScanContent() {
	...
	const handleScan = useCallback(
		(result: { rawValue: string }[]) => {
			...
			if (scanStatus.type === "scanning" || boothCheckin.isPending) return;
	...
	return (
		<div className="flex flex-1 flex-col items-center px-6 py-8">
			...
			{/* Visitor count */}
			<h2 className="font-serif text-2xl text-[var(--text-primary)] text-center mt-2">{statsLoading ? "載入中…" : `${visitorCount} 人來過`}</h2>
			...
			>
				...
				<span className="font-serif text-lg font-bold text-[var(--text-primary)] whitespace-nowrap">玩家 / 攤位</span>
			</button>
		</div>
	);
}
