...
export default function FriendsPage() {
	...
	const [selectedFriend, setSelectedFriend] = useState<FriendPublicProfile | null>(null);
	...
	const pageFriends = (friends ?? []).slice(page * PAGE_SIZE, page * PAGE_SIZE + PAGE_SIZE);

	return (
		<div className="bg-[var(--bg-primary)] px-6 py-6">
			{/* Title */}
			<h1 className="text-[var(--text-primary)] text-3xl font-serif font-bold text-center mb-6">好友列表</h1>

			{total === 0 ? (
				<p className="text-center font-serif text-[var(--text-secondary)] mt-10">還沒有好友，快去掃描 QR Code 認識新朋友吧！</p>
			) : (
				<>
					{/* Friend list */}
					<ul className="flex flex-col gap-3">
						{pageFriends.map(friend => (
							<li key={friend.id} className="rounded-2xl bg-[var(--bg-secondary)] shadow-sm">
								<button type="button" onClick={() => setSelectedFriend(friend)} className="flex w-full cursor-pointer items-center gap-4 px-5 py-4 text-left">
									{friend.avatar ? (
										<img src={friend.avatar} alt={friend.nickname} className="h-12 w-12 rounded-full object-cover flex-shrink-0" />
									) : (
									...
									<div className="flex min-w-0 flex-col">
										<span className="font-serif font-semibold text-[var(--text-primary)] truncate">{friend.nickname}</span>
										<span className="text-sm text-[var(--text-secondary)]">第 {friend.current_level} 關</span>
									</div>
									<span className="ml-auto text-xs font-semibold text-[var(--text-secondary)]">點擊看名牌</span>
								</button>
							</li>
						))}
					</ul>
