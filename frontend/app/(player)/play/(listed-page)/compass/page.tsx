...
export default function CompassPage() {
  ...
  return (
    <div className="bg-[var(--bg-primary)] px-6 pt-6 pb-[calc(var(--navbar-height)+1.5rem)]">
      ...
      <p className="mt-2 text-center text-sm font-serif text-[var(--text-secondary)]">
        你的指南針小組：{currentUser.group}
      </p>
      ...
      {tab === "scan" ? (
        ...
        <div className="mt-6 space-y-3">
          {(members ?? []).length === 0 ? (
            <p className="text-center font-serif text-[var(--text-secondary)] mt-8">
              目前還沒有其他指南針夥伴。
            </p>
          ) : (
            (members ?? []).map((member) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate font-serif font-semibold text-[var(--text-primary)]">
                    {member.nickname}
                  </p>
                  <p className="text-sm text-[var(--text-secondary)]">
                    第 {member.current_level} 關
                  </p>
                </div>
                ...
                <span
                  className={`rounded-full px-3 py-1 text-xs font-semibold ${
                    member.checked_in
                      ...
                      : "bg-[rgba(93,64,55,0.15)] text-[var(--text-secondary)]"
                  }`}
                >
