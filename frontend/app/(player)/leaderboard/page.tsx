...
function RefreshIcon({ className }: { className?: string }) {
	return (
		<svg className={className} width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
			<path d="M21 2v6h-6" />
	...
	return (
		<div className={`relative rounded-xl px-5 py-3.5 transition-colors ${hasCurrentUser ? "bg-[var(--bg-header)] ring-2 ring-[var(--accent-gold)]" : "bg-[var(--bg-header)]/85"}`}>
			<div className="flex items-center gap-4">
				{/* Left: rank number */}
				<div className="text-[var(--text-light)] font-semibold text-lg tabular-nums shrink-0 w-8 text-center">{rank}.</div>
...
export default function LeaderboardPage() {
	...
	const topRankCount = useMemo(() => new Set(topEntries.map(entry => entry.rank)).size, [topEntries]);

	/* Split nearby into above/below the current user for the "You ▼" divider */
	const currentGroupIdx = me ? groupedEntries.findIndex(group => group.entries.some(entry => isSameEntry(me, entry))) : -1;
	const showDivider = mode === "nearby" && currentGroupIdx > 0;
	const aboveGroups = showDivider ? groupedEntries.slice(0, currentGroupIdx) : [];
	const belowGroups = showDivider ? groupedEntries.slice(currentGroupIdx) : groupedEntries;
			...
			<div className="mb-6 flex items-center justify-between">
				...
				<div className="flex items-center gap-2">
					...
					>
						{mode === "top" ? (
							<>
								<span className="font-bold text-[var(--text-primary)]">排名前 {topRankCount || 10} 名</span> / 我附近的玩家
							</>
						) : (
							<>
								排名前 {topRankCount || 10} 名 / <span className="font-bold text-[var(--text-primary)]">我附近的玩家</span>
							</>
						)}
					</button>
				</div>
				...
				{showDivider ? (
					<>
						{aboveGroups.map((group, idx) => (
							<LeaderboardRow key={`rank-group-${group.rank}-${idx}`} entries={group.entries} me={me} onSelectEntry={setSelectedEntry} />
						))}
						<YouDivider />
					...
					<>
						...
						{mode === "top" && me && !groupedEntries.some(group => group.entries.some(entry => isSameEntry(me, entry))) && (
							<>
								<YouDivider />
								<LeaderboardRow entries={[me]} me={me} onSelectEntry={setSelectedEntry} />
							</>
						)}
					</>
				)}
