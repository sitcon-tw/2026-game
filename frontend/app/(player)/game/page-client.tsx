...
function LevelNoteIcon({ className }: { className?: string }) {
	return (
		<svg className={className} viewBox="0 0 65 65" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden>
			<path
...
function LevelsPageContent({ user, leaderboard }: { user: ClientUser | null; leaderboard: RankResponse | null }) {
	...
	const completedSet = new Set(Array.from({ length: currentLevel }, (_, i) => i + 1));
	const unlockedLevels = Array.from({ length: unlockLevel }, (_, i) => i + 1);
	const progressPercent = unlockLevel > 0 ? Math.round((currentLevel / unlockLevel) * 100) : 0;
		...
		<div className="px-5 pb-10 pt-6">
			<section className="rounded-[20px] bg-[var(--bg-secondary)]/70 p-4 shadow-sm">
				...
				<div className="mt-3 flex items-center justify-between gap-3">
					<div className="text-sm text-[var(--text-secondary)]">已完成關卡 / 已解鎖關卡</div>
				...
				<div className="mt-3 h-2.5 w-full overflow-hidden rounded-full bg-[rgba(255,255,255,0.7)]">
					<div className="h-full rounded-full bg-[var(--accent-gold)]" style={{ width: `${progressPercent}%` }} />
				</div>
				<div className="mt-2 text-xs text-[var(--text-secondary)]">已解鎖 {unlockLevel} 關</div>
			</section>

			<section className="mt-6">
				<div className="mb-3 text-sm font-semibold text-[var(--text-secondary)]">已解鎖關卡</div>
					...
					{unlockedLevels.map(level => {
						...
						return (
							<div key={level} className="flex flex-col items-center gap-2">
								{isPlayable ? (
									<Link href={`/game/${level}`} className="group flex flex-col items-center gap-2" aria-label={`第 ${level} 關${isCompleted ? "，可重新挑戰" : "，可挑戰"}`}>
									...
									<div className="flex flex-col items-center gap-2 cursor-not-allowed" aria-label={`第 ${level} 關，尚未開放`}>
										<div className="relative h-16 w-16 opacity-30">
											...
											<span className="absolute -top-1 left-1 text-sm font-semibold text-[var(--text-secondary)]">{level}</span>
										</div>
										<span className="text-xs font-medium text-[var(--text-secondary)]/70">未開放</span>
									</div>
								)}
							</div>
						);
...
export default function LevelsPageClient({ user, leaderboard }: { user: ClientUser | null; leaderboard: RankResponse | null }) {
	return (
		<Suspense fallback={<LoadingSpinner />}>
			<LevelsPageContent user={user} leaderboard={leaderboard} />
		</Suspense>
	);
}
